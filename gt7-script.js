/* =========================================================
   ...
   the live sheet can't be reached (e.g. offline, URL typo, or
   the sheet was unpublished). Keep gt7-data.csv alongside this
...
const FALLBACK_DATA_URL = "gt7-data.csv";
...
async function loadData() {
  ...
  try {
    ...
    console.warn("Could not reach the live Google Sheet, using local fallback data instead:", err);
...
function isValidStart(r) {
  if (!r.start) return false;
  const s = r.start.trim().toUpperCase();
  if (s === "" || s === "N/A" || s === "NA" || s === "DNS") return false;
  const n = parseInt(s);
  return !isNaN(n) && n > 0;
}
